...
for epoch in tqdm(range(total_ppo_epochs)):
    ...
    for _ in range(config['batch_size']):
        ...
        tokens = gpt2_tokenizer.encode(random_prompt) #list of ids
        batch['tokens'].append(tokens)
        batch['query'].append(random_prompt)
    query_tensors = [torch.tensor(t).long().to(device) for t in batch["tokens"]]  #128个[seq_len]的一维向量

    t = time.time()
    response_tensors = []
    for i in range(config['batch_size']):
        gen_len = config['gen_len']
        response = gpt2_model.generate(query_tensors[i].unsqueeze(dim=0),       # response是包含输入的prompt的
                                       max_new_tokens=gen_len, **gen_kwargs)
        response_tensors.append(response.squeeze()[-gen_len:])  #[ ] -gen_len: 是因为gpt2_model.generate会输出上文吗，这里做剔除，看起来是这样
    batch['response'] = [gpt2_tokenizer.decode(r.squeeze()) for r in response_tensors]  #这个squeeze好像是多余的
    timing['time/get_response'] = time.time() - t

    t = time.time()
    texts = [q + r for q,r in zip(batch['query'], batch['response'])]           # 计算正向/负向情感得分
    pipe_outputs = sentiment_pipe(texts) #[ ]文档中pipeline的输出内容有哪些
    rewards = []
